...
Prompt templates never fetch data directly from Redis or PostgreSQL.
...
The Resume Parser returns structured resume data used to build Resume Intelligence.
...
The Resume Parser does **not** generate the Technology Graph or Interview Contexts directly.

Those structures are generated by the Resume Intelligence Builder after validation.
...
The evaluator scores only the current answer for the active topic.

It does not calculate:

- Overall interview score.
- Hiring recommendation.
- Final strengths and weaknesses across all topics.

Those responsibilities belong to the Final Evaluation Prompt.
...
Schema validation is performed using typed output schemas inside `internal/interview/engine/schemas`.

Only schema-valid responses are returned to the Interview Engine.
...
- Prompt templates remain provider-independent and communicate only through structured JSON contracts.
